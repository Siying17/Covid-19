...
make_history <- function(N, M, k, omega, theta, pa, pb) {
  ...
  n <- sum(h_lab == 1) # number captured
  print(paste("n =", n))
  n_2 <- sum(h_hosp == 1, na.rm = T) # number recaptured
  print(paste("n_2 =", n_2))
  ...
  pb_init <- theta_init <- 1 / (mean_lag_hosp/2 + 1)
  
  lag_init <- floor(lag_init_hosp/2)
  lag_init[is.na(lag_init)] <- rnbinom(sum(is.na(lag_init)), 1, theta_init)
